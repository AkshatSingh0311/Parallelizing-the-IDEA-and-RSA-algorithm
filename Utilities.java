...
String decimal = Integer.toBinaryString(decimalNumber); int l = decimal.length();
if (l < 8) {
for (int i = 0; i <= 7 - l; i++) {
// System.out.println("adding 0"); decimal = "0".concat(decimal);
}
}
return decimal;
}

private String decimalToBinary16(long decimalNumber) { String decimal = Long.toBinaryString(decimalNumber); int l = decimal.length();
if (l < 16) {
...
private int binaryToDecimal16(String binaryString) { int decimal = 0;
for (int i = 15; i >= 0; i--) {
decimal += ((int) (binaryString.charAt(15 - i)) - 48)
* (int) Math.pow(2, i);
}
return decimal;
}

private int binaryToDecimal8(String binaryString) { int decimal = 0;
for (int i = 7; i >= 0; i--) {
decimal += ((int) (binaryString.charAt(7 - i)) - 48) * (int) Math.pow(2, i);
}
return decimal;
}private String MultiplicationModulo(String x, String y) { long n1 = binaryToDecimal16(x);
long n2 = binaryToDecimal16(y);
return decimalToBinary16(((n1 * n2) % 65537));
}

private String AdditionModulo(String x, String y) { int n1 = binaryToDecimal16(x);
...
if ((decimal * i) % 65537 == 1) { return decimalToBinary16(i);
}
}
...
temp = temp.concat(Character.toString((char)
...
public String doIDEASequenceLoop(String p1, String p2, String p3, String p4, Utilities utils,
ArrayList<String[]> keySet) {
String result1, result2, result3, result4, result5, result6, result7, result8, result9, result10, result11,
result12, result13, result14; String[] currentKeySet;
...
// System.out.println(p1 + " - " + p2 + " - " + p3 + " - " + p4); return p1 + p2 + p3 + p4;
}
}
