...
import java.util.ArrayList; public class KeyGenerator {
public Key generateKeySet(String originalKey, boolean isEncryption) {
originalKey = rounder(originalKey);
String eightRounds = originalKey.substring(0, 768); String halfRound = originalKey.substring(768); ArrayList<String[]> myList = new ArrayList<>(); String[] temp;
int v = 0;
for (int i = 0; i < 8; i++) { temp = new String[6];
for (int j = 0; j < 6; j++) {
...
myList.add(temp); if (!isEncryption) {
...
for (int i = 1; i <= 6; i++) {
originalKey = circularLeftShiftBy25(originalKey); if (i != 6) {
...
finalNumber = number.substring(25).concat(finalNumber); return finalNumber;
}
...
String[][] store = new String[9][6]; for (int i = 0; i < 9; i++) {
